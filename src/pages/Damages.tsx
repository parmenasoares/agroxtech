...
const DAMAGE_TABLE_CANDIDATES = ["damage_reports", "damages", "damage_requests"];
...
const Damages = () => {
  ...
  const loadRequests = async (uid: string) => {
    ...
    try {
      ...
      let loaded: DamageRequest[] = [];

      for (const table of DAMAGE_TABLE_CANDIDATES) {
      ...
      setRequests([]);
      if (lastError && !isMissingBackendObjectError(lastError)) {
        throw lastError;
      }
    } catch (error) {
      toast({
