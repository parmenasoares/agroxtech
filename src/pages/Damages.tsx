  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const adaptersToTry = activeAdapter
        ? [activeAdapter, ...DAMAGE_ADAPTERS.filter((adapter) => adapter.table !== activeAdapter.table)]
        : DAMAGE_ADAPTERS;
              ...
              <CardContent className="space-y-4">
                {moduleUnavailable && (
                  <p className="text-sm text-muted-foreground">
                    Módulo de danos em configuração. Solicite ao suporte a atualização da base de dados.
                  </p>
                )}
