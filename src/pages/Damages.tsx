...
const Damages = () => {
  ...
  const loadRequests = async (uid: string) => {
    ...
    try {
      const { data, error } = await supabase
        .from("damage_reports")
        .select("id, created_at, description, location_text, status, response, responded_at, photo_path")
        .eq("user_id", uid)
        .order("created_at", { ascending: false });

      if (error) throw error;

      setRequests((data ?? []) as DamageRequest[]);
    } catch (error) {
      toast({
        title: t("error"),
  ...
  const uploadPhoto = async (uid: string) => {
    ...
    const { error } = await supabase.storage.from("damage-reports").upload(filePath, photoFile, { upsert: true });
    if (error) throw error;
  ...
  const getPhotoUrl = (path: string) => supabase.storage.from("damage-reports").getPublicUrl(path).data.publicUrl;
