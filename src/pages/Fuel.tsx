...
const Fuel = () => {
  ...
  const insertFuelRecord = async (payload: Record<string, unknown>) => {
    ...
    throw lastError ?? new Error("Fuel module backend not available.");
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isMissingBackendObjectError(error)) {
        setFeedback("Módulo de combustível em configuração no banco de dados.", "error");
      } else {
