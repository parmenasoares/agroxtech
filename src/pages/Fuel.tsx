...
const Fuel = () => {
  ...
  const uploadImage = async () => {
    ...
    const { error } = await supabase.storage.from("fuelings").upload(filePath, file, { upsert: true });
    if (error) throw error;

    return supabase.storage.from("fuelings").getPublicUrl(filePath).data.publicUrl;
  };
